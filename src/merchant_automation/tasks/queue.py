...
from merchant_automation.tasks.models import Attachment, Task, TaskEvent, TaskMetricBucket, TaskMetrics, TaskResult, TaskStatus
...
def _attachment_update_to_row(attachment: Attachment) -> tuple:
	row = _attachment_to_row(attachment)
	return (*row[1:], row[0])
...
class TaskQueue:
	...
	async def update_attachment(self, attachment: Attachment) -> None:
		...
		self._ensure_started()
		assert self._db is not None
		await self._db.execute(_UPDATE_ATTACHMENT_SQL, _attachment_update_to_row(attachment))
		await self._db.commit()
